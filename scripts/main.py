    ...
    for year in years:
        for plan in plans:
            for scenario in scenarios:
                ...
                output_dir = os.path.join(args.output_dir, f"MP2023_{simulations_mapping[scenario]}_{simulations_mapping[plan]}_C000_U00_V00_SLA_{year}_insured_{args.insurance}")
                os.makedirs(output_dir, exist_ok=True)

                # Extract geographic attributes
                #logging.info("Extracting geographic attributes...")
                if args.existing_geo:
                    geo_file = os.path.join(output_dir, f"MP2023_{simulations_mapping[scenario]}_{simulations_mapping[plan]}_C000_U00_V00_SLA_{year}_RR2_geo.csv")
                    ...
                    geo_save_path = os.path.join(output_dir , f"MP2023_{simulations_mapping[scenario]}_{simulations_mapping[plan]}_C000_U00_V00_SLA_{year}_RR2_geo.csv")
                ...
                filename =  f"MP2023_{simulations_mapping[scenario]}_{simulations_mapping[plan]}_C000_U00_V00_SLA_{year}_RR2_prem.csv"
